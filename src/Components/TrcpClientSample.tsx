...
const TrcpClientSample = () => {
	...
	return (
		<>
			...
			<button
				onClick={async () => {
					const data1 = await trpcClient.updateAlbum.mutate({ albumId: 3, title: "33333", subtitle: "444444", isShared: true });
					console.log(data1)
				}}
			>
				앨범 변경
			...
			<button
				onClick={async () => {
					const data1 = await trpcClient.insertImage.mutate([{ albumId: 4, path: "/test/is/test", size: 102400, width: 100, height: 100 }]);
					console.log(data1)
				}}
			>
				이미지 추가
